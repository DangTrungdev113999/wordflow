...
export function ContentExtractor({ loading, title, text }: Props) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 space-y-4">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-xs text-gray-600 dark:text-gray-400">Extracting vocabulary with AI</p>
        </div>
  ...
  return (
    <div className="rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50 p-4">
      ...
      <p className="text-xs text-gray-700 dark:text-gray-300 line-clamp-3">{text.slice(0, 200)}...</p>
    </div>
  );
}
...
function Step({ label, done, active }: { label: string; done: boolean; active: boolean }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <span className={`text-xs ${done ? 'text-emerald-600' : active ? 'text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400'}`}>
        {label}
      </span>
    </div>
  );
