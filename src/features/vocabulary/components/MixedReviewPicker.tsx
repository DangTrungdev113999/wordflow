...
import { Shuffle, Play, Clock, AlertTriangle, Dices, LayoutList, Layers, HelpCircle, FileText, Lightbulb } from 'lucide-react';
...
const SOURCE_FILTERS: { key: MixedSourceFilter; label: string; icon: React.ReactNode; color: string; activeColor: string }[] = [
  { key: 'due', label: 'Đến hạn', icon: <Clock size={16} />, color: 'text-red-500', activeColor: 'bg-red-500 text-white shadow-red-200 dark:shadow-red-900/50' },
  { key: 'weak', label: 'Từ yếu', icon: <AlertTriangle size={16} />, color: 'text-amber-500', activeColor: 'bg-amber-500 text-white shadow-amber-200 dark:shadow-amber-900/50' },
  { key: 'random', label: 'Ngẫu nhiên', icon: <Dices size={16} />, color: 'text-violet-500', activeColor: 'bg-violet-500 text-white shadow-violet-200 dark:shadow-violet-900/50' },
  { key: 'all', label: 'Tất cả', icon: <LayoutList size={16} />, color: 'text-blue-500', activeColor: 'bg-blue-500 text-white shadow-blue-200 dark:shadow-blue-900/50' },
];
...
export function MixedReviewPicker({ stats, loading, initialFilter, onStart }: MixedReviewPickerProps) {
  ...
  return (
    ...
    >
      ...
      {/* Diversity Tip */}
      {!loading && stats.topicsWithProgress <= 2 && (
        <div className="flex items-start gap-2.5 p-3 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50">
          <Lightbulb size={16} className="text-amber-500 mt-0.5 shrink-0" />
          <p className="text-xs text-amber-700 dark:text-amber-300">
            Học thêm topics để Mixed Review hiệu quả hơn!
          </p>
        </div>
      )}
